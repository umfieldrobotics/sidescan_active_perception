...
class imvp_classifier(pl.LightningModule):
    def __init__(self, train_dataset, val_dataset, num_workers, 
        ...
        #     nn.ReLU(),
        #     nn.Dropout(0.5),
        #     nn.Linear(256, len(volume_cutoffs))
        # )
        self.class_fc = nn.Linear(1000, self.num_classes)
        self.volume_fc = nn.Linear(1000, len(volume_cutoffs))
        ...
        self.val_logits = []
        self.val_volume_logits = []
        self.val_volume_lbls = []
        self.val_lbls = []
        self.train_preds = []
        self.train_gts  = []
    ...
    def forward(self, images, feat=False):
        ...
        class_prediction = self.class_fc(features) #only use the last output of the rnn
        volume_prediction = self.volume_fc(features)
        return class_prediction, volume_prediction
    ...
    def training_step(self, batch, batch_idx):
        try:
            ...
            class_pred, volume_pred = self(data) #model_out is Bx1 now
            # model_out = model_out.reshape(N, L, -1).mean(dim=1)
            # losses = self.loss(model_out, batch, grad=True)
            self.train_preds.append(torch.argmax(volume_pred, dim=1))
            self.train_gts.append(volume_lbls)
            train_loss = self.loss(volume_pred, volume_lbls.long()) + 100*self.loss(class_pred, lbls.long()) 
            reg_term = 0.0
    ...
    def on_train_epoch_end(self):
        ...
        train_Acc = (torch.cat(self.train_preds, dim=0) == torch.cat(self.train_gts, dim=0).int()).float().mean()
        self.log('train_volume_accuracy', train_Acc, sync_dist=True)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.val_volume_logits = torch.cat(self.val_volume_logits, dim=0)
        self.val_volume_lbls = torch.cat(self.val_volume_lbls, dim=0)

        volume_preds = torch.softmax(self.val_volume_logits, dim=1)
        volume_preds = torch.argmax(volume_preds, dim=1)

        volume_acc = (volume_preds.squeeze() == self.val_volume_lbls.int()).float().mean()
        self.log('val_volume_accuracy', volume_acc, sync_dist=True)

        #calculate val loss 
        val_loss = self.loss(self.val_volume_logits, self.val_volume_lbls.long()) + 100*self.loss(self.val_logits, self.val_lbls.long())
        ...
        # self.cms = np.zeros((self.num_angles, self.num_classes, self.num_classes))
        # self.num_val_iters = 0 
        # freeze_layers_resnet(self.image_encoder, 150)

    def validation_step(self, batch, batch_idx):
        data, lbls, volume_lbls, files = batch
        # if coords.shape[0] == 1:
        #     coords = coords.repeat(2,1,1)
        #     pcd = pcd.repeat(2,1,1)
        #     apc = apc.repeat(2,1,1)
        ...
        class_pred, volume_pred = self(data, feat=True)
        self.val_volume_logits.append(volume_pred)
        self.val_volume_lbls.append(volume_lbls)
        self.val_logits.append(class_pred) #ensemble
